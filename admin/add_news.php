...
if($size > 9000000){
	...
	$pass = "news_photos/".$name.".$ext";
	move_uploaded_file($_FILES['fileField']['tmp_name'],$pass);
	$image = $name.".$ext";
	
	if(ExecuteQuery("insert into news (news_id,title,content,time_update,day,month,year,filename,status)
		values(NULL,'".addslashes($_POST['title'])."','".addslashes($_POST['content'])."','".date('d F Y')."','".date('d')."','".date('F')."','".date('Y')."','".$image."','0')")){
		$message=successMsg('News uploaded successfully ');
                    ...
                    <div class="row-fluid">
                        ...
                        <div class="block">
                            <div class="navbar navbar-inner block-header">
                                <div class="muted pull-left"><b style="font-size:14px; color:#000">Add News</b></div>
                            </div>
