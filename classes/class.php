...
class connection{
	...
	var $db='prospe_prosperity_cup';
	var $localhost='localhost';
	var $password='[L42l9UKW(hyg7';
	var $username='prospe';
	var $connect;
	var $database;
	public function connectTodatabase(){
	 ...
	 $path="http://www.prosperitycup.com/";
	 return $path;
 } 
...
public function sendpaymentMail($name,$recieverid){
	...
	$rows=$connection->arrayFetch($result);
	 ...
	 foreach($rows as $row){
								...
								$output.='<li class="col-md-4 sportsmagazine-the-team">';
                                  $output.=' <figure>
                                            <a href="blog-detail.html"> <img src="cpanel/news_photos/'.$row['filename'].'" alt=""></a>                                           <figcaption>
                                                <span><small>The Team</small></span>
                                                <a href="'.$connection->url().'news/'.$row['day'].'/'.$row['month'].'/'.$row['year'].'/'.str_replace(' ', '-',$row['title']).'/'.$row['news_id'].'"" class="sportsmagazine-link-btn"><i class="fa fa-link"></i></a>
                                            </figcaption>
                                        </figure>';
                                     $output.='   <section>';
                                         $output.='   <h2><a  href="'.$connection->url().'/news/'.$row['day'].'/'.$row['month'].'/'.$row['year'].'/'.str_replace(' ', '-',$row['title']).'/'.$row['news_id'].'">'.$row['title'].'</a></h2>';
                                          $output.='<p>'.strip_tags( substr($row['content'],0,70)).'</p>';
                                      $output.='  </section>
...
function getPaymentstomake($id,$pck){
   ...
   $output.=' </div>';
	...
	$offset =  ($page_number  - 1) * $recordperpage;
	//$query= "SELECT * FROM `gallery` order by photo_id DESC limit $offset, $recordperpage";
	
	
	$result_lgas= "SELECT * FROM `gallery` order by photo_id DESC limit $offset, $recordperpage";
	$result=$connection->retrieve($result_lgas);
	$rows=$connection->arrayFetch($result);
	 foreach($rows as $row){
								   ...
								   $output.=' <li>
                                        <figure>';
                                          $output.='   <a data-fancybox-group="group" href="'.$connection->url().'admin/gallery/'.$row['filename'].'" class="fancybox"><img src="'.$connection->url().'admin/gallery/'.$row['filename'].'" ><i class="icon-signs23"></i></a>';
                                             $output.='<span>'.ucwords($row['title']).'</span>';
                                             $output.='<figcaption>';
                                                 $output.='<h6><a href="#">'.$row['caption'].'</a></h6>';
                                               $output.='<time datetime="">'.ucwords($row['title']).'</time>';
                                             $output.='</figcaption>
                                        </figure>
                                    </li>';
	...
	$result_lgas= "SELECT * FROM `videos` order by video_id DESC limit $offset, $recordperpage";
	$result=$connection->retrieve($result_lgas);
	$rows=$connection->arrayFetch($result);
	 foreach($rows as $row){
								   ...
								   $output.='<li>
                                        <figure>';
                                          $output.='   <a data-fancybox-group="group" href="'.$connection->url().'/video/'.str_replace(' ', '-',$row['title']).'/'.$row['video_id'].'"><img src="'.$connection->url().'admin/video_photos/'.$row['img'].'" ></a>';
                                             $output.='<span>Play Now</span>';
                                             $output.='<figcaption>';
                                                 $output.='<h6><a href="'.$connection->url().'video/'.str_replace(' ', '-',$row['title']).'/'.$row['video_id'].'">'.ucwords($row['title']).'</a></h6>';
                                               $output.='<time datetime="">Play Now</time>';
                                             $output.='</figcaption>
                                        </figure>
                                    </li>';
	...
	$rows=$connection->arrayFetch($result);
	 foreach($rows as $row){
		...
		$connection->selectDatabase();
	$output="";
	$result_lgas= "select * from videos where video_id !=$id order by video_id desc limit 3 ";
	$result=$connection->retrieve($result_lgas);
	$rows=$connection->arrayFetch($result);
	 foreach($rows as $row){
			   ...
			   $output.='<li>
                                        <div class="sportsmagazine-popular-post">';
                                         $output.='   <figure><a href="'.$connection->url().'video/'.str_replace(' ', '-',$row['title']).'/'.$row['video_id'].'"><img src="'.$connection->url().'admin/video_photos/'.$row['img'].'" alt=""></a></figure>';
                                           $output.=' <div class="sportsmagazine-popular-post-text">';
                                              $output.='<h5><a href="'.$connection->url().'video/'.str_replace(' ', '-',$row['title']).'/'.$row['video_id'].'">'.$row['title'].'</a></h5>';
                                              $output.='<time datetime="2008-02-14 20:00">'.$row['post_time'].'</time>';
                                           $output.=' </div>
                                            <span></span>
                                        </div>
                                    </li> ';
	...
	function recent_news(){
		...
		$connection->selectDatabase();
	$output="";
	$result_lgas= "select * from news order by news_id desc limit 3 ";
	$result=$connection->retrieve($result_lgas);
	$rows=$connection->arrayFetch($result);
	 foreach($rows as $row){
			   ...
			   $output.='<li>
                                        <div class="sportsmagazine-popular-post">';
                                         $output.='   <figure><a href="'.$connection->url().'update/'.$row['day'].'/'.$row['month'].'/'.$row['year'].'/'.str_replace(' ', '-',$row['title']).'/'.$row['news_id'].'"><img src="'.$connection->url().'admin/news_photos/'.$row['filename'].'" alt=""></a></figure>';
                                           $output.=' <div class="sportsmagazine-popular-post-text">';
                                              $output.='<h5><a href="'.$connection->url().'update/'.$row['day'].'/'.$row['month'].'/'.$row['year'].'/'.str_replace(' ', '-',$row['title']).'/'.$row['news_id'].'">'.$row['title'].'</a></h5>';
                                              $output.='<time datetime="2008-02-14 20:00">'.$row['time_update'].'</time>';
                                           $output.=' </div>
                                            <span></span>
                                        </div>
                                    </li> ';
	...
	$offset =  ($page_number  - 1) * $recordperpage;
	$query= "SELECT * FROM `news`  where status=1 order by time_update DESC limit $offset, $recordperpage";
	$result=$connection->retrieve($query);
	$rows=$connection->arrayFetch($result);
	 ...
	 foreach($rows as $row){
									  ...
									  $output.='<li class="col-md-4">';
                                         $output.='<figure>';
                                           $output.='<a  href="'.$connection->url().'update/'.$row['day'].'/'.$row['month'].'/'.$row['year'].'/'.str_replace(' ', '-',$row['title']).'/'.$row['news_id'].'"><img src="admin/news_photos/'.$row['filename'].'" alt=""></a>';
                                             $output.='<figcaption>';
                                               
                                                 $output.='<a href="'.$connection->url().'update/'.$row['day'].'/'.$row['month'].'/'.$row['year'].'/'.str_replace(' ', '-',$row['title']).'/'.$row['news_id'].'" class="sportsmagazine-link-btn"><i class="fa fa-link"></i></a>';
                                            $output.=' </figcaption>
                                        </figure>
                                        <section>';
                                            $output.=' <h2><a  href="'.$connection->url().'update/'.$row['day'].'/'.$row['month'].'/'.$row['year'].'/'.str_replace(' ', '-',$row['title']).'/'.$row['news_id'].'">'.$row['title'].'</a></h2>';
                                             $output.='<p>'.strip_tags( substr($row['content'],0,70)).'</p>';
                                         $output.='</section>
	...
	$rows=$connection->arrayFetch($result);
	 ...
	 foreach($rows as $row){
									  ...
									  $output.='<li class="col-md-4">';
                                        ...
                                        <section>';
                                            $output.=' <h2 style="font-size:20px"><a  href="'.$connection->url().'update/'.$row['day'].'/'.$row['month'].'/'.$row['year'].'/'.str_replace(' ', '-',$row['title']).'/'.$row['news_id'].'">'.$row['title'].'</a></h2>';
                                             $output.='<p>'.strip_tags( substr($row['content'],0,70)).'</p>';
                                         $output.='</section>
